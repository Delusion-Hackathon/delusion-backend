...
from delusion.users.models import Anomaly, User, Node
import random
from delusion.users.permissions import IsUserOrReadOnly
from delusion.users.serializers import (
...
class NodeListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        data["node_type"] = random.choices(
            ["server", "router", "switch", "firewall", "storage"]
        )
        data["status"] = random.choice(["active", "inactive", "maintenance"])
        data["os_version"] = random.choice(
            [
                "Ubuntu 20.04",
                "CentOS 8",
                "Debian 10",
        ...
        if (
            data["cpu_usage"] > 90
            or data["memory_usage"] > 90
            or data["disk_usage"] > 90
        ):
            anomaly = Anomaly.objects.create(
                node=serializer.instance, description="High resource usage detected"
            )
